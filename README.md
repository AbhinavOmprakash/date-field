...
* [`src/date_field/`](src/date_field/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/date_field/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `date_field` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/date_field/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns date-field.example
  (:require [date-field.config :as config])
